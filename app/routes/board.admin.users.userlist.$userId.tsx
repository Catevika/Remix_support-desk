...
export const loader: LoaderFunction = async ({ params }) => {
	...
	return data<LoaderData>({ user, services });
};
...
export default function adminUserIdRoute() {
	...
	const isUpdating = Boolean(fetcher.formData?.get("intent") === "update");
	const isDeleting = Boolean(fetcher.formData?.get("intent") === "delete");
			...
			{user ? (
				<main className="form-container-center">
					<div className="form-content">
						<fetcher.Form method="post" className="form" key={user?.id}>
							<input
								type="hidden"
								name="redirectTo"
							...
							<div className="inline">
								...
								<Link to="/board/admin/users/userlist/index" className="btn">
									Back to User List
								</Link>
								<button
									type="submit"
				...
				return (
					<div className="error-container" style={{ fontSize: "1.5rem" }}>
						<div className="form-container form-container-message form-content">
							...
							<Link
								to={`/board/admin/users/userlist/${userId}`}
